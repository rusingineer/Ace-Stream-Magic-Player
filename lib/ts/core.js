	...
	ajax: function(conf) {
	    ...
	    */
        ...
        if(typeof conf.data === "object") {
            if(conf.method === "GET") {
                ...
                for(var k in conf.data) {
                    a.push(k + "=" + encodeURIComponent(conf.data[k]));
                }
                var s = a.join("&");
                if(url.indexOf("?") === -1) {
                    url += "?" + s;
        ...
        if(typeof GM_xmlhttpRequest === "function") {
            //TODO: pass headers (need to convert from object to array)
            GM_xmlhttpRequest({
                    method: conf.method,
                    url: url,
                    data: postData,
                    ...
                    onload: function(response) {
                        if(response.status == 200) {
                            if(typeof conf.success === 'function') {
                                ...
                                conf.success.call(null, response);
                            }
                        }
                        else {
                            if(typeof conf.error === 'function') {
                                conf.error.call(null);
                            }
                        }
                    },
        ...
        xhr.onreadystatechange = function() {
            if(xhr.readyState == 4) {
                ...
                TorrentStream.Utils.log("ajax: request finished: status=" + xhr.status + " url=" + url);
                ...
                if(xhr.status == 200) {
                    if(typeof conf.success === 'function') {
                        ...
                        conf.success.call(null, response, xhr);
                    }
                }
                else {
                    if(typeof conf.error === 'function') {
        ...
        timer = setTimeout(function() {
                xhr.abort();
                TorrentStream.Utils.log("ajax: request timed out: " + url);
        }, conf.timeout * 1000);
    },
