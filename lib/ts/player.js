TorrentStream.FakePlayer = function(gContainer, gConf)
{
    ...
    function getContentIdByUrl(conf) {
        ...
        var requestData = {
            ...
            arguments: {
                url: conf.url,
                developer_id: conf.developerId || 0,
                affiliate_id: conf.affiliateId || 0,
                zone_id: conf.zoneId || 0
            }
        };
                
        TorrentStream.Utils.ajax({
                method: "POST",
    ...
    function getEngineVersion(callback) {
        TorrentStream.Utils.ajax({
                method: "GET",
                url: "http://localhost:6878/webui/api/service?method=get_version",
                timeout: 5,
                dataType: "json",
                success: function(response) {
                    try {
                        // for testing
                        console.log("getEngineVersion: success", response);
                        if(response.error) {
                            throw "api error: " + response.error;
                        }
                        if(!response.result) {
                        ...
                        log("getEngineVersion: got engine version: " + response.result.code);
                        if(typeof callback === "function") {
                            callback.call(null, response.result.code);
                        }
                    }
                    catch(e) {
                        log("getEngineVersion: request failed: " + e);
                        if(typeof callback === "function") {
                ...
                error: function() {
                    log("getEngineVersion: request failed");
                    if(typeof callback === "function") {
                        callback.call(null, null);
                    }
                }
...
TorrentStream.Player = function(container, conf)
{
    ...
    function embedPlugin(container, useInternalPlaylist, style, bgColor, fontColor, callback)
    {
        ...
        if(typeof XPCNativeWrapper !== 'undefined') {
            conf.firefoxUnwrapEmbedObjects = true;
        }
    ...
    function afterEmbed(embedObject) {
        ...
        // for testing
        console.log("embedObject: embedObject", embedObject);
